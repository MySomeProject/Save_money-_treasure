...
@interface SetobijiaViewController ()
@property (weak, nonatomic) IBOutlet UIWebView *webview;
@property(nonatomic,retain)Lloadview *loadview;
@end

@implementation SetobijiaViewController
...
- (IBAction)back:(UIButton *)sender {
    [self.webview stringByEvaluatingJavaScriptFromString:@"backWebView()"];
}
- (BOOL)webView:(UIWebView*)webView shouldStartLoadWithRequest:(NSURLRequest*)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ([urlString containsString:@"ios"]) {
        ...
        if ([[ioscanshudic objectForKey:@"closeWindow"] isEqualToString:@"1"]) {
            [self.navigationController popToViewController:[self.navigationController.viewControllers objectAtIndex:3] animated:YES];
        }else if([[ioscanshudic objectForKey:@"openWindow"] isEqualToString:@"1"]){
            [self performSegueWithIdentifier:@"Bijiatoorder" sender:urlString];
        }
        return NO;
    }else{
        return YES;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"Bijiatoorder"]) {
        OrderH5ViewController* ovc = segue.destinationViewController;
        ovc.url = sender;
    }
}
